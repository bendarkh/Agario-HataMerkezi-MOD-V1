// ==UserScript==
// @name         HATA MERKEZİ MOD
// @namespace    http://www.hatamerkezi.com
// @version      1.7
// @description  Agar.io W - Shift - Harita Modu hatamerkezi.com
// @author       Berker HÜRDOĞAN
// @license      HATA MERKEZİ MOD
// @match        http://agar.io/*
    ...
    function miniMapInit() {
        ...
        if ($('#mini-map-wrapper').length === 0) {
            var wrapper = $('<div>').attr('id', 'mini-map-wrapper').css({
                ...
                width: 200,
                height: 200,
                background: 'url(http://i.hizliresim.com/Xo5YM5.png)'
            });
        ...
        if ($('#mini-map-options').length === 0) {
            ...
            var help_btn = $('<a>')
                .addClass('btn')
                .text('Hata Merkezi Agar.io Harita')
                .on('click', function(e) {
                    window.open('http://www.hatamerkezi.com');
                    help_btn.blur();
                    return false;
                })
                .appendTo(form_group);
